...
void Image::drawImage(const Image& img1, const Image& img2, int x, int y, float factor)
{
	if (x > (int)width || y > (int)height || (x + (int)img1.width) < 0 || (y + (int)img1.height) < 0)
		return; //outside

	if (x > (int)width || y > (int)height || (x + (int)img2.width) < 0 || (y + (int)img2.height) < 0)
		return; //outside

	int startx = clamp(x, 0, (int)width);
	int starty = clamp(y, 0, (int)height);
	int endx = clamp(x + img1.width, 0, (int)width);
	int endy = clamp(y + img1.height, 0, (int)height);

	//iterate 
	for (int i = startx; i < endx; ++i)
		for (int j = starty; j < endy; ++j)
		{
			const Color &c1 = img1.getPixelRef(i - x, j - y);
			const Color &c2 = img2.getPixelRef(i - x, j - y);

			// only blend rgb, not alpha
			char r = c1.r * factor + c2.r * (1.0 - factor);
			char g = c1.g * factor + c2.g * (1.0 - factor);
			char b = c1.b * factor + c2.b * (1.0 - factor);

			const Color result(r, g, b, c1.a); // get alpha from first image 
			...
			if (result.a == 255)
				...
				pixels[pos] = blendColors(result, pixels[pos]);
		}
}
...
void Image::drawImage(const Image& img1, const Image& img2, int x, int y, int imgx, int imgy, int imgw, int imgh, float factor)
{
	if (x > (int)width || y > (int)height || (x + (int)imgw) < 0 || (y + (int)imgh) < 0)
		return; //outside
	if (x > (int)width || y > (int)height || (x + (int)imgw) < 0 || (y + (int)imgh) < 0)
	...
	imgx = clamp(imgx, 0, (int)img1.width);
	imgy = clamp(imgy, 0, (int)img1.height);
	imgw = clamp(imgw, 0, (int)img1.width - imgx);
	imgh = clamp(imgh, 0, (int)img1.height - imgy);
	int startx = clamp(x, 0, (int)width);
	int starty = clamp(y, 0, (int)height);
	int endx = clamp(x + imgw, 0, (int)width);
	int endy = clamp(y + imgh, 0, (int)height);
	...
	for (int i = startx; i < endx; ++i)
		for (int j = starty; j < endy; ++j)
		{
			...
			const Color &c1 = img1.getPixel(i - x + imgx, j - y + imgy);
			const Color &c2 = img2.getPixel(i - x + imgx, j - y + imgy);
...
void Image::maskAlpha(int alpha)
{
	unsigned int l = width * height;
	for (unsigned int i = 0; i < l; ++i)
	{
