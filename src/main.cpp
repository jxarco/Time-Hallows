...
SDL_Window* createWindow(const char* caption, int width, int height, bool fullscreen = false)
{
	...
	//create the window
	SDL_Window *window = SDL_CreateWindow(caption, 300, 100, width, height, SDL_WINDOW_OPENGL|SDL_WINDOW_RESIZABLE|
...
void mainLoop()
{
	...
	while (!game->must_exit)
	{
		...
		game->totalTimeElapsed += elapsed_time;
		game->frame++;
		frames_this_second++;
...
int main(int argc, char **argv)
{
	...
	SDL_Window* window = createWindow("THE TIME HALLOWS", size, size);
	if (!window)
		return 0;

	//launch the game (game is a global variable)
	game = new Game(size, size, window);
